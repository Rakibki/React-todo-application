...
const ShowTodos = ({todos, handleUpdate, hanndleDelete, TodoComplate, filterText}) => {
  ...
  useEffect(() => {
    ...
    if(filterText === 'inConplate') {
      const inComplatedTodos = todos.filter((item) => item.isComplete === false) 
  ...
  return (
    <div id={styles.todoContainer}>
        ...
        {
            filterTodos.map((todo, index) => <Todo TodoComplate={TodoComplate} handleUpdate={handleUpdate} hanndleDelete={hanndleDelete} todo={todo} key={index} />)
        }  
    </div>
  )
}
